     ...
     <div>
      <div>
        <div className="flex  text-center space-x-4 w-full p-10">
          <div className={`${courses.length > 0 ? 'w-1/3' : 'mx-auto'} bg-white bg-opacity-90 p-8 rounded-2xl shadow-xl`}>
              ...
              .filter((reservation) => {
                ...
                <div key={index} className="w-1/3 mb-8">
                  <div className="bg-white bg-opacity-90 p-8 rounded-2xl shadow-xl h-full" >                    
                    <div className="text-center">
                      <div className="space-y-4">
                        <p><span className="font-bold">{reservation.id}. {reservation.Course.name}</span></p>
                        <p>{reservation.Course.userId}</p>
                        <p>{reservation.Availability.date} {reservation.Availability.startTime} - {reservation.Availability.endTime}</p>
                        <p><span className="font-bold">Precio: </span>${reservation.Course.price.toLocaleString()}</p>
                      </div>
                    </div>
                    {!reservation.isCancelled 
                      ? <button onClick={() => handleClickCancelReservation(reservation)} className="w-full py-2 bg-red-500 hover:bg-red-700 text-white font-bold rounded-full focus:outline-none focus:shadow-outline mt-4">
